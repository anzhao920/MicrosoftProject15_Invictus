...
FILE *fp,*fpout,*fplist, *fpwoa, *metaDataF;
...

{
 ...
 if ((fp = fopen(filename,"rb+\0")) == NULL)
  ...
  if ( *xchoice == 'I' ) {
   ...
   fscanf(metaDataF, "%f",&xinterv1);
   printf("Enter deepest depth\n");
   fscanf(metaDataF, "%f",&xinterv2);
  }
...
/*   GET USER INFORMATION (NUMBER OF CASTS, OUTPUT FILE NAME) */
  ...
  if ( (s = fscanf(metaDataF, "%d", &ncast)) == 0 ) ncast=0;
