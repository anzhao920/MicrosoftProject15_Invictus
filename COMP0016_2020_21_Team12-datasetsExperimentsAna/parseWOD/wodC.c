/* Source : https://data.nodc.noaa.gov/woa/WOD/PROGRAMS/wodC.c
C Program designed as an example on how to read
the World Ocean Database (WOD) native format data.

The programs prompts the user to input a WOD native format
data file to read and number of casts to print out.
wodC to a user named file, all variables included in all desired casts from the input
WOD native format file in a easily readable form.

This main program calls subroutines to read and load into arrays
the WOD data and passed back to the main program.  */
